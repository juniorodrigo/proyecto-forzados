...
export async function GET() {
	try {
		...
		const { recordset } = await pool.request().query("SELECT * FROM MAE_RIESGO_A WHERE ESTADO = 1");
...
export async function POST(request: Request) {
	try {
		...
		const result = await pool
			...
			.query(
				"INSERT INTO MAE_RIESGO_A (DESCRIPCION, USUARIO_CREACION, USUARIO_MODIFICACION, FECHA_CREACION, FECHA_MODIFICACION, ESTADO) VALUES (@descripcion, @usuario, @usuario, GETDATE(), GETDATE(), 1)"
			);
...
export async function DELETE(request: Request) {
	try {
		...
		const result = await pool
			...
			.input("id", id)
			.input("usuario", usuario)
			.query("UPDATE MAE_RIESGO_A SET ESTADO = 0, USUARIO_MODIFICACION = @usuario, FECHA_MODIFICACION = GETDATE() WHERE RIESGOA_ID = @id");
...
export async function PUT(request: Request) {
	try {
		...
		const result = await pool
			...
			.input("descripcion", descripcion)
			.input("usuario", usuario)
			.query("UPDATE MAE_RIESGO_A SET DESCRIPCION = @descripcion, USUARIO_MODIFICACION = @usuario, FECHA_MODIFICACION = GETDATE() WHERE RIESGOA_ID = @id");
