...
export async function GET() {
	try {
		...
		const { recordset } = await pool.request().query("SELECT * FROM MAE_ROL WHERE ESTADO = 1");

		const turnos = recordset.map((singleValue) => {
			return {
				id: singleValue.ROL_ID,
...
// 			.query("INSERT INTO AREA (DESCRIPCION, USUARIO_CREACION, USUARIO_MODIFICACION, FECHA_CREACION, FECHA_MODIFICACION, ESTADO) VALUES (@descripcion, @usuario, @usuario, GETDATE(), GETDATE(), 1)");
