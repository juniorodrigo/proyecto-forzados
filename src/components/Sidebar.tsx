...
const Sidebar: React.FC = () => {
  ...
  const itemRolesIncludesUserRoles = (
    itemRoles: number[],
    userRoles: number[]
  ) => {
  ...
  return (
    <div
      className={`bg-[#001d39] border-r border-gray-300 flex flex-col transition-all duration-300 ease-in-out ${
        open ? "w-64" : "w-16"
      } h-screen z-40`}
    >
        ...
        <ul>
          {menuItems
            .filter(
              (item) =>
                user &&
                itemRolesIncludesUserRoles(
                  item.roles,
                  Object.keys(user.roles).map(Number)
                )
            )
            .map((item) => (
                  ...
                  <div
                    className={`flex items-center p-4 cursor-pointer text-white ${
                      activeToggle === item.id
                        ? "bg-[#133e6a] text-gray-100"
                        : "hover:bg-[#00264d] hover:text-gray-200"
                    } ${open ? "" : "justify-center"}`}
                  >
                    <span className="text-[#c8a064] transition-transform duration-200 group-hover:scale-110">
                      {item.icon}
                    </span>
                    {open && (
